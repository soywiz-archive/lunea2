...
version (Windows) {
	...
	// Used for API >= 1.2
	void glBindFunc(void** ptr, char[] funcName) {
		void* func = wglGetProcAddress(std.string.toStringz(funcName));
		if (!func) throw(new Exception(std.string.format("Can't bind '%s'", funcName)));
		*ptr = func;
	}	
	
	// Used for API == 1.1
	void glBindFuncBase(void** ptr, char[] funcName) {
		version (Windows) {
			void* func = GetProcAddress(opengl_dll, std.string.toStringz(funcName));
			if (!func) throw(new Exception(std.string.format("Can't bind '%s'", funcName)));
			*ptr = func;
		} else {
	...
	private void glInitWindows() {
		debug (debug_gl_ver) writefln("glInitWindows");
...
char[] glBindFuncBaseMix(char[] t) { return "glBindFuncBase(cast(void**)&" ~ t ~ ", \"" ~ t ~ "\");"; }
char[] glBindFuncMix(char[] t) { return "glBindFunc(cast(void**)&" ~ t ~ ", \"" ~ t ~ "\");"; }
bool   glCatch(void function() glUsingVer) { try { glUsingVer(); return true; } catch (Exception e) { debug (debug_gl_ver) writefln(e); return false; } }
...
class glShaderException : Exception { this(char[] s) { super(s); } }
...
class glShader {
	...
	this(GLenum type, char[] program) {
		this(type);
		set(program);
	}
	
	void set(char[] program) {
		int strl = program.length;
		char* str = program.ptr;
	...
	void compile() {
		...
		if (!r) {
			...
			error.length = r;
			throw(new glShaderException(error));
		}
	}
}
...
class glFragmentShader : glShader {
	...
	this(char[] program) { super(GL_FRAGMENT_SHADER, program); }
}

class glVertexShader : glShader {
	this() { super(GL_VERTEX_SHADER); }
	this(char[] program) { super(GL_VERTEX_SHADER, program); }
}
...
class glProgram {
	...
	glAttrib getAttrib(char[] name, bool except = false) {
		GLuint aid = glGetAttribLocation(id, std.string.toStringz(name));
		if (except && aid == -1) throw(new Exception(std.string.format("Attribute '%s' isn't active", name)));
		return new glAttrib(id, aid);
	}
	
	glUniform getUniform(char[] name, bool except = false) {
